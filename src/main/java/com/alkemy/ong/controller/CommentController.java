...
public class CommentController {
    ...
    //Update Comment by id only for User owner or Admin role
    @PutMapping("/{id}")
    public ResponseEntity<?> updateComment(Authentication auth,@RequestBody @Valid CommentDto commentDto, @PathVariable String id) {
        if (this.commentService.existId(id)) {
            throw  new ParamNotFound("There is no comment with the entered Id");
        }
        try {
            this.commentService.update(auth,commentDto, id);
            return new ResponseEntity<>(HttpStatus.ACCEPTED);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
        }
